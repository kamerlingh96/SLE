  ...
  function student_profile($student_id)
  {
    ...
    function class_routine($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $data['year']           = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
            $data['pdf']            = $data['section_id'].$data['class_id'].$data['year'].".pdf";
            ...
            $array = array(
               ...
               'class_id'      => $data['class_id'],
               'year'          => $data['year'],
               'pdf'           => $data['section_id'].$data['class_id'].$data['year'].".pdf"
            );
            $validation = duplication_of_class_routine_on_create($array);
            if ($validation == 1) {

                //archivo
                $mi_archivo = 'file_name';
                $config['upload_path'] = "./uploads/horarios";
                $config['file_name'] = $data['section_id'].$data['class_id'].$data['year'];
    ...
    function attendance_update($class_id = '' , $section_id = '' , $timestamp = '', $recompensa = '', $demerito = '')
    {
        $running_year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
    ...
    function compra($class_id = '')
  	{
  		...
  		$page_data['page_name']  	= 'manage_purchase';
  		$page_data['page_title'] 	= "Administrar compra". " - ".get_phrase('class')." : ".
  											$this->crud_model->get_class_name($class_id);
  		$page_data['class_id'] 	= $class_id;
  		$this->load->view('backend/index', $page_data);
  	}

    function compraStudent($student_id = '',$class_id = '')
  	{
      ...
      $page_data['page_name']  = 'student_purchase';
  		$page_data['page_title'] = "Estudiante - Compra";
      $page_data['student_id']  = $student_id;
  		$this->load->view('backend/index', $page_data);
  	}

    function generate_purchase($class_id = '',$student_id = ''){
      $data['year']               = $this->db->get_where('settings' , array('type'=>'running_year'))->row()->description;
      ...
      redirect(site_url('admin/compra/'.$class_id), 'refresh');
    }
    ...
    function frase(){
      if ($this->session->userdata('admin_login') != 1)
          redirect(site_url('login'), 'refresh');
      $page_data['page_name']  = 'frase';
      $page_data['page_title'] = "Frase";
      $idFrase = 1;
      $result =  $this->db->query("SELECT * FROM frases WHERE id = '" . $idFrase . "' LIMIT 1");
      $page_data['frases'] = $result->row();
      $this->load->view('backend/index', $page_data);
    }
    ...
    function actualizar_frase(){
      ...
      $data['frase'] = $this->input->post('frase');
      $idFrase = 1;
      $this->db->where('id' , $idFrase);
      $this->db->update('frases' , $data);


      redirect(site_url('admin/frase'), 'refresh');
